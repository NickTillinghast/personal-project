import React, { Component } from "react";
import "./Cora.css";

export default class Cora extends Component {
  render() {
    return (
      <div>
        <div>
          <div className="cora-hero-div">
            <div className="cora-text">
              <div>
                This is my first born daughter who has made me father. I love
                being a father and wouldn't have anything without my family.
              </div>
            </div>
            <img
              className="cora-hero"
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573666187/eblp0d1dvakptmfde0w4.jpg"
              alt="none"
            />
          </div>
        </div>
        <div className="cora-container">
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445291/Sized%20down%20for%20personal%20project/uhccefwr6ny2kbruiptq.jpg"
              alt="none"
            />
          </div>
          <div className="cora-vert">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445091/Sized%20down%20for%20personal%20project/qhlythzhxgrojtiukqjn.jpg"
              alt="none"
            />
          </div>
          <div className="cora-horiz">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445139/Sized%20down%20for%20personal%20project/oges4m4qndyje58tk0c4.jpg"
              alt="none"
            />
          </div>
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445078/Sized%20down%20for%20personal%20project/ssjgg17kfpr70u2bfn9p.jpg"
              alt="none"
            />
          </div>
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445410/Sized%20down%20for%20personal%20project/vjvmdwb2sg1eoksgczx6.jpg"
              alt="none"
            />
          </div>
          <div className="cora-big">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573444925/Sized%20down%20for%20personal%20project/hzuwtfsnesawlhd84tsp.jpg"
              alt="none"
            />
          </div>
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573444949/Sized%20down%20for%20personal%20project/scnfhvbesxx3gjczqsqz.jpg"
              alt="none"
            />
          </div>
          <div className="cora-vert">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445223/Sized%20down%20for%20personal%20project/a0h4s2mwqnmuihnczoo2.jpg"
              alt="none"
            />
          </div>
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573444894/Sized%20down%20for%20personal%20project/d23xj5afkewdkxax6rgu.jpg"
              alt="none"
            />
          </div>
          <div className="cora-horiz">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445070/Sized%20down%20for%20personal%20project/k9zvafhzmrxbzyvrtobv.jpg"
              alt="none"
            />
          </div>
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445185/Sized%20down%20for%20personal%20project/pjvorsazebrxyexyzg7z.jpg"
              alt="none"
            />
          </div>
          <div className="cora-big">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573444965/Sized%20down%20for%20personal%20project/jms3uuizj8pax7hjanoo.jpg"
              alt="none"
            />
          </div>
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573444972/Sized%20down%20for%20personal%20project/evohstwwcaihnek6fyxx.jpg"
              alt="none"
            />
          </div>
          <div className="cora-horiz">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573444918/Sized%20down%20for%20personal%20project/dkx5t5kdibzvnpzy02nn.jpg"
              alt="none"
            />
          </div>
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445101/Sized%20down%20for%20personal%20project/b7qt7rdf2chp3dbyuups.jpg"
              alt="none"
            />
          </div>
          <div className="cora-big">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573444933/Sized%20down%20for%20personal%20project/sciccbfxv4lkhhh4apam.jpg"
              alt="none"
            />
          </div>
          <div className="cora-plain">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445594/v0opnf8e5whszxgsux1t.jpg"
              alt="none"
            />
          </div>
          <div className="cora-vert">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445542/gvhpfitmrrz02fnexlcr.jpg"
              alt="none"
            />
          </div>
          <div className="cora-vert">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445442/Sized%20down%20for%20personal%20project/d5hrlq0icbub6iaiocka.jpg"
              alt="none"
            />
          </div>
          <div className="cora-vert">
            <img
              src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573445452/Sized%20down%20for%20personal%20project/cmasxb1tog4tidilbeyh.jpg"
              alt="none"
            />
          </div>
        </div>
      </div>
    );
  }
}
