import React, { Component } from "react";
import snowhouse from "./snowhouse.mp4";
import "./Portfolio.css";

export default class Portfolio extends Component {
  render() {
    return (
      <div className="about-main">
        <div className="top">
          <div className="video-box">
            <video className="video" autoPlay loop muted preload>
              <source src={snowhouse} type="video/mp4" />
            </video>
          </div>
          <h1 className="port-text">Winter Wonderland Wedding. </h1>
          <h2 className="port-text">
            Love can keep you warm any time of season
          </h2>
        </div>
        <div className="middle">
          <div>
            <div className="grid-container">
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095063/Sized%20down%20for%20personal%20project/gvchl6gwcoxxlehaufbo.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095062/Sized%20down%20for%20personal%20project/rslkhsnb4ddgy4jxhh5a.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095062/Sized%20down%20for%20personal%20project/ggkqiq5e5uyixeu9sqik.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095063/Sized%20down%20for%20personal%20project/gdvisqa1sfspi96k2hy5.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095063/Sized%20down%20for%20personal%20project/ohxsmbr8xv2pfslhrcra.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573505268/Sized%20down%20for%20personal%20project/soizodd9herhy6detmmu.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095063/Sized%20down%20for%20personal%20project/gvchl6gwcoxxlehaufbo.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095064/Sized%20down%20for%20personal%20project/jxluzvh1ghndtyx95zka.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095064/Sized%20down%20for%20personal%20project/hmvgu3xmtc6pc7n8qvqm.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573505268/Sized%20down%20for%20personal%20project/zb1yfuxvzgcnqb5lkfm2.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573505287/Sized%20down%20for%20personal%20project/cumwifdfsoslxkxwggsw.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573505208/Sized%20down%20for%20personal%20project/rn8ro5gqld0ismr0dala.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095066/Sized%20down%20for%20personal%20project/gjfhrtgy93dvo5lrfhby.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095067/Sized%20down%20for%20personal%20project/fqtyazn0dci2zfyqcpfb.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095067/Sized%20down%20for%20personal%20project/pzosyt0ozysvoedlo2oc.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573505225/Sized%20down%20for%20personal%20project/ydotoxvmq4icp1sd92ec.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095067/Sized%20down%20for%20personal%20project/ivbkbc5j7dr76ajkmqhv.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095068/Sized%20down%20for%20personal%20project/jxwavwtctefakxckl5sp.jpg"
                  alt="none"
                />
              </div>

              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095068/Sized%20down%20for%20personal%20project/emdnlvdxjleayqpojijd.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095068/Sized%20down%20for%20personal%20project/bsgyjvbpw1pl9hhl1clf.jpg"
                  alt="none"
                />
              </div>
              <div className="grid-item">
                <img
                  className="grid-image"
                  src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573095064/Sized%20down%20for%20personal%20project/ngvnjzki91temgyw3svf.jpg"
                  alt="none"
                />
              </div>
            </div>
          </div>
        </div>
        <h1 className="port-text">Rustic Barn setting with an amazing view</h1>
        <div>
          <img
            className="hero"
            src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241199/Sized%20down%20for%20personal%20project/xsvajxzywieyemkoqxuz.jpg"
            alt="none"
          />
        </div>
        <div className="second-grif">
          <div className="grid-container">
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241186/Sized%20down%20for%20personal%20project/dzgdvlcofujjepfxj4xp.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241186/Sized%20down%20for%20personal%20project/sjvnhjbovzwksc8yvppv.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241188/Sized%20down%20for%20personal%20project/xh4vydvzaeuowfe6qteq.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241188/Sized%20down%20for%20personal%20project/nln7hfysrsbavdw6jjrl.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241189/Sized%20down%20for%20personal%20project/nqis5vntsnqqqpotsf9f.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241189/Sized%20down%20for%20personal%20project/okjjgrorvhjalk8drtqb.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241186/Sized%20down%20for%20personal%20project/yewxpcaxukm124jstrdz.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241190/Sized%20down%20for%20personal%20project/gqe3mnq1elpdn705nsid.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241189/Sized%20down%20for%20personal%20project/crngz1hy4vkvqm0head4.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241190/Sized%20down%20for%20personal%20project/nd93ej3iopg0vi4znfif.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241190/Sized%20down%20for%20personal%20project/viq9uoe1s3uh7e3kmwc4.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241196/Sized%20down%20for%20personal%20project/uvrlcdbg4djqjhgzkeak.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241198/Sized%20down%20for%20personal%20project/bmpuozdwo9wdyhvdu0qc.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241198/Sized%20down%20for%20personal%20project/nhwvha7sb5x9my56wvel.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241188/Sized%20down%20for%20personal%20project/xbdub7ukpxe2tn7wkgez.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241198/Sized%20down%20for%20personal%20project/dzhqmlycvzo94n5aiqz3.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241199/Sized%20down%20for%20personal%20project/swf0h76jbji3dmpk3zti.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241199/Sized%20down%20for%20personal%20project/zl61iaw5rgpi6mquoloq.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241201/Sized%20down%20for%20personal%20project/mbl1dlxf3dfwexlofwep.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241203/Sized%20down%20for%20personal%20project/umztanffvsup77owuiau.jpg"
                alt="none"
              />
            </div>
            <div className="grid-item">
              <img
                className="grid-image"
                src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241203/Sized%20down%20for%20personal%20project/rmwtycecflal5ccibty6.jpg"
                alt="none"
              />
            </div>
          </div>
        </div>
        <div>
          <img
            className="hero"
            src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241199/Sized%20down%20for%20personal%20project/xsvajxzywieyemkoqxuz.jpg"
            alt="none"
          />
        </div>
        <div className="second-grif">
          <div className="grid-container">
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
          </div>
        </div>
        <div>
          <img
            className="hero"
            src="https://res.cloudinary.com/dqpatwz6c/image/upload/v1573241199/Sized%20down%20for%20personal%20project/xsvajxzywieyemkoqxuz.jpg"
            alt="none"
          />
        </div>
        <div className="second-grif">
          <div className="grid-container">
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
            <div className="grid-item">
              <img className="grid-image" src="" alt="none" />
            </div>
          </div>
        </div>
      </div>
    );
  }
}
